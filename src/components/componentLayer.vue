<template>
    <div class="component-layer" box-border border="t-1 gray-2">
        <div class="layer-item" @click="handleClick(item.id)"
            :class="item.id == currentComponent?.id ? 'bg-blue-100 color-blue-500' : ''"
...
const { lowCanvasState, currentComponent } = storeToRefs(store)

const reseverComponentData = computed(() => {
    return cloneDeep(lowCanvasState.value.data).reverse()
})
