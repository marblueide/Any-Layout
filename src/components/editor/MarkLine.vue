<template>
    <div class="mark-line">
        <div class="line" v-for="item in lines" absolute :class="item.includes('x') ? 'xline' : 'yline'" bg-blue-2
            :style="getShapeStyle(lineState[item].style)" v-show="lineState[item].isShow">
        </div>
    </div>
</template>
...
const showLine = (isDown: boolean, isLeft: boolean) => {
    //@ts-ignore
    const currentStyle = getComponentRotatedStyle(cloneDeep(currentComponent.value?.style))
    //@ts-ignore
    const { left: curLeft, top: curTop, right: curRight, bottom: curBottom, width: curWidth, height: curHeight, rotate } = currentStyle
    //@ts-ignore
    ...
    lowCanvasData.value.forEach(data => {
        ...
        //@ts-ignore
        const { left, top, width, height } = getComponentRotatedStyle(cloneDeep(data.style))
        const conditions: {
            [k in 'top' | 'left']: {
            ...
            top:
                [
                    ...
                    {
                        line: 'xb',
                        isNearly: isNearly(top + height, curTop) || isNearly(top + height, curBottom),
                        lineValue: top + height,
                        curValue: isNearly(top + height, curTop) ? top + height : top + height - curHeight
                    }
                ],
            left: [
                {
                ...
                {
                    line: 'yr',
                    isNearly: isNearly(left + width, curLeft) || isNearly(left + width, curRight),
                    lineValue: left + width,
                    curValue: isNearly(left + width, curLeft) ? left + width : left + width - curWidth
                }
            ]
        }
        Object.keys(conditions).forEach((key) => {
            conditions[key as 'top' | 'left'].forEach(condition => {
                if (condition.isNearly) {
                    ...
                    lineState[condition.line].style[key as 'top' | 'left'] = condition.lineValue
                    store.setCurrentComponentStyle({
                        [key]: rotate != 0 ? transformCurComponnet(key, condition.curValue, currentStyle) : condition.curValue
                    })
                }
            })
...
const transformCurComponnet = (key: 'top' | 'left', value: number, currentStyle: StyleValue) => {
    //@ts-ignore
